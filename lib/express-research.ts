...
export function createExpressResearchResult(input:{business:Pick<Business,"name"|"category"|"address">;depth:ResearchDepth;officialUrl?:string|null;pages?:ExpressResearchPage[];commonsImages?:ExpressResearchImage[];warnings?:string[]}):ExpressResearchResult{
 ...
 const promptResearch:MasterPromptResearch={businessDescription:merged.description,summary:`${pages.length} offizielle Seite(n), ${images.length} Bildkandidat(en) und ${links.length+1} Prüfpfade vorbereitet. Google Maps wird nur verlinkt; Fotos und Bewertungen werden nicht gescrapt.`,services:merged.serviceHints,evidence};
