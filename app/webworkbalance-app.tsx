...
import { quickAuditFromWebsiteReport } from "@/lib/website-quality";
import {
  calculateLeadScore,
  categories,
  DEFAULT_PRICES,
  ...
  type LeadStatus,
  type LeadTask,
  type OfficialWebsiteVerification,
  type PriceSettings,
  type SearchCenter,
  type StoredLead,
...
type LeadSortMode = "updated" | "lead-score" | "website-opportunity";
type LeadWebsiteFilter = "all" | "scanned" | "unscanned" | "missing";
...
const REGIONAL_SCAN_BUDGET_MS = 13_000;
...
export function WebWorkBalanceApp({ currentUser }: { currentUser: AppUser }) {
  ...
  const [leadSortMode, setLeadSortMode] = useState<LeadSortMode>("updated");
  const [leadWebsiteFilter, setLeadWebsiteFilter] = useState<LeadWebsiteFilter>("all");
  const [websiteBatchScanning, setWebsiteBatchScanning] = useState(false);
  const [websiteBatchProgress, setWebsiteBatchProgress] = useState("");
  const [websiteScanSkippedIds, setWebsiteScanSkippedIds] = useState<Set<string>>(new Set());
  ...
  const filteredLeads = useMemo(() => {
    ...
    const filtered = leads.filter((lead) => {
      ...
      if (leadWebsiteFilter === "scanned" && !lead.websiteReport) return false;
      if (leadWebsiteFilter === "unscanned" && (!lead.website || lead.websiteReport)) return false;
      if (leadWebsiteFilter === "missing" && lead.website) return false;
      return !query || `${lead.name} ${lead.category} ${lead.address}`.toLowerCase().includes(query);
    });
    return [...filtered].sort((first, second) => {
      if (leadSortMode === "lead-score") return second.score - first.score || second.updatedAt.localeCompare(first.updatedAt);
      if (leadSortMode === "website-opportunity") {
        const firstQuality = first.websiteReport?.overallScore ?? Number.POSITIVE_INFINITY;
        const secondQuality = second.websiteReport?.overallScore ?? Number.POSITIVE_INFINITY;
        return firstQuality - secondQuality || second.score - first.score;
      }
      return second.updatedAt.localeCompare(first.updatedAt);
    });
  }, [leads, leadQuery, leadSortMode, leadStatusFilter, leadWebsiteFilter]);

  const websiteScanQueue = useMemo(() => leads.filter((lead) => (
    Boolean(lead.website) &&
    !lead.websiteReport &&
    !lead.isDemo &&
    !websiteScanSkippedIds.has(lead.id) &&
    (!lead.claimedById || lead.claimedById === activeUser.id)
  )), [activeUser.id, leads, websiteScanSkippedIds]);
  ...
  async function acquireRadarResults(
    ...
    try {
      ...
      if (shared.businesses.length && !shared.clientFallback && !shared.stale) {
        return {
          ...
          cached: true,
          stale: false,
          notice: shared.cached
            ? `Schnell aus dem gemeinsamen Team-Cache geladen${shared.provider ? ` · ${shared.provider}` : ""}.`
            : `Server-Radar live geladen${shared.provider ? ` · ${shared.provider}` : ""}${shared.sampled ? " · großer Radius als begrenztes Suchfeld" : ""}.`,
        };
      }
    ...
    try {
      ...
      const savedBusinesses = leads
        .filter((lead) => !lead.isDemo && lead.source === "OpenStreetMap" && (targetCategory === "all" || lead.categoryKey === targetCategory))
        .map((lead) => ({
          ...lead,
          distanceKm: Number(haversineKm(targetCenter.lat, targetCenter.lon, lead.lat, lead.lon).toFixed(1)),
        }))
        .filter((lead) => lead.distanceKm <= targetRadiusKm + 0.25)
        .sort((first, second) => first.distanceKm - second.distanceKm);
      const cachedBusinesses = staleShared?.businesses.length ? staleShared.businesses : staleDevice?.result.businesses ?? [];
      const fallbackBusinesses = cachedBusinesses.length ? cachedBusinesses : savedBusinesses;
      if (fallbackBusinesses.length) {
        return {
          ...
          cached: true,
          stale: true,
          notice: cachedBusinesses.length
            ? "Alle freien Live-Endpunkte waren kurzzeitig nicht erreichbar. Damit du weiterarbeiten kannst, wird der letzte gespeicherte Suchstand angezeigt."
            : "Alle freien Live-Endpunkte waren kurzzeitig nicht erreichbar. Damit du weiterarbeiten kannst, werden echte, bereits im Team gespeicherte OSM-Leads aus diesem Suchgebiet angezeigt.",
        };
      }
      throw error;
    }
  ...
  async function runSearch() {
    ...
    try {
      ...
      const diagnostics = error instanceof RadarDirectError && error.diagnostics.length
        ? ` Geprüft: ${error.diagnostics.slice(0, 3).join(" · ")}. Die drei kostenlosen Kartendienste antworten gerade nicht; versuche es später erneut oder starte mit einem kleineren Radius.`
        : "";
  ...
  async function loadRegionalList() {
    ...
    try {
      ...
      let regionalBudgetReached = false;
      const regionalScanStartedAt = Date.now();
      for (const [index, place] of REGIONAL_SEARCH_CENTERS.entries()) {
        if (index > 0 && Date.now() - regionalScanStartedAt >= REGIONAL_SCAN_BUDGET_MS) {
          regionalBudgetReached = true;
          diagnostics.push("Regionales Zeitbudget erreicht; bereits geladene Treffer bleiben nutzbar");
          break;
        }
        ...
        try {
          ...
          let result = cachedArea?.result;
          if (!result) {
            try {
              const serverArea = await requestJson<RadarServerResponse>("/api/search", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(request),
              });
              if (serverArea.businesses.length && !serverArea.clientFallback && !serverArea.stale) {
                result = {
                  businesses: serverArea.businesses,
                  provider: serverArea.provider ?? "Server-Radar",
                  fieldsSearched: serverArea.segments ?? 1,
                  fieldsPlanned: serverArea.segments ?? 1,
                  partial: false,
                  sampled: Boolean(serverArea.sampled),
                  diagnostics: [],
                };
              }
            } catch {
              // Der Geräte-Direktmodus bleibt der kostenlose Fallback.
            }
          }
          result ??= await searchOpenStreetMapDirect(request, (message) => setRegionalNotice(`${place.name}: ${message}`));
      ...
      if (!collected.length) {
        ...
        const cachedFallback = staleShared?.businesses.length ? staleShared.businesses : staleLocal?.result.businesses ?? [];
        const savedRegional = leads
          .filter((lead) => !lead.isDemo && lead.source === "OpenStreetMap")
          .map((lead) => ({ ...lead, distanceKm: Number(haversineKm(DEFAULT_CENTER.lat, DEFAULT_CENTER.lon, lead.lat, lead.lon).toFixed(1)) }))
          .filter((lead) => lead.distanceKm <= 80)
          .sort((first, second) => first.name.localeCompare(second.name, "de"));
        const fallback = cachedFallback.length ? cachedFallback : savedRegional;
        ...
        setRegionalNotice(cachedFallback.length
          ? "Freie Live-Endpunkte waren nicht erreichbar. Der letzte gespeicherte Regionalstand bleibt verfügbar."
          : "Freie Live-Endpunkte waren nicht erreichbar. Echte, bereits im Team gespeicherte OSM-Leads aus Nürnberg und der Region bleiben verfügbar.");
        toast.info(`${fallback.length} gespeicherte Firmen aus Nürnberg & Region geladen`);
        return;
      }
      ...
      setRegionalNotice(`${completedAreas}/${REGIONAL_SEARCH_CENTERS.length} Teilregionen direkt geladen · ${regionalResult.provider}${diagnostics.length ? " · Anbieterwechsel aktiv" : ""}${regionalBudgetReached ? " · Lauf bewusst zeitlich begrenzt" : ""}.`);
  ...
  async function patchLead(id: string, patch: Partial<Pick<StoredLead, "status" | "priority" | "notes" | "nextAction" | "followUpAt" | "websiteStatus" | "audit" | "websiteReport" | "activities">>) {
  ...
  async function scanSavedLeadWebsites() {
    const queue = websiteScanQueue.slice(0, 8);
    if (!queue.length) return toast.info("Keine ungescannten offiziellen Websites in deiner freien Arbeitsliste.");
    setWebsiteBatchScanning(true);
    let scanned = 0;
    let skipped = 0;
    const skippedIds: string[] = [];
    for (const [index, lead] of queue.entries()) {
      setWebsiteBatchProgress(`${index + 1}/${queue.length} · ${lead.name}`);
      try {
        const scan = await requestJson<WebsiteBatchScanResponse>("/api/website-quality", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ business: { name: lead.name, address: lead.address, phone: lead.phone, email: lead.email, source: lead.source, sourceUrl: lead.sourceUrl, website: lead.website } }),
        });
        if (!scan.report) {
          skipped += 1;
        ...
        const audit = quickAuditFromWebsiteReport(scan.report, lead.audit);
        const updated = await requestJson<{ lead: StoredLead }>("/api/leads", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id: lead.id, websiteReport: scan.report, websiteStatus: "exists", audit }),
        });
        setLeads((current) => current.map((item) => item.id === lead.id ? updated.lead : item));
        scanned += 1;
      } catch {
        skipped += 1;
        skippedIds.push(lead.id);
    ...
    if (skippedIds.length) setWebsiteScanSkippedIds((current) => new Set([...current, ...skippedIds]));
    setWebsiteBatchProgress("");
    setWebsiteBatchScanning(false);
    if (scanned) toast.success(`${scanned} offizielle Website${scanned === 1 ? "" : "s"} geprüft${skipped ? ` · ${skipped} nicht eindeutig oder blockiert` : ""}`);
    else toast.info("Keine Website konnte eindeutig geprüft werden. Blockierte oder nicht bestätigte Seiten bleiben ohne Score.");
  }
      ...
      <Tabs value={view} onValueChange={(value) => navigate(value as AppView)}>
        ...
        <TabsContent value="leads" className="mx-auto max-w-[1380px] px-4 py-5 sm:px-6">
          ...
          <section className="glass-panel mt-5 rounded-[1.75rem] p-4 sm:p-5">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(280px,1fr)_210px_210px_220px]"><label className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={leadQuery} onChange={(event) => setLeadQuery(event.target.value)} placeholder="Firma, Branche oder Ort suchen" className="h-11 w-full rounded-xl border border-white/10 bg-black/15 pl-10 pr-3 outline-none focus:border-[#d7b56d]/50" /></label><Select value={leadStatusFilter} onValueChange={setLeadStatusFilter}><SelectTrigger className="h-11 w-full border-white/10"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Alle Status</SelectItem>{leadStatuses.map((status) => <SelectItem key={status} value={status}>{status}</SelectItem>)}</SelectContent></Select><Select value={leadWebsiteFilter} onValueChange={(value) => setLeadWebsiteFilter(value as LeadWebsiteFilter)}><SelectTrigger className="h-11 w-full border-white/10"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Alle Websites</SelectItem><SelectItem value="scanned">Website geprüft</SelectItem><SelectItem value="unscanned">Website noch prüfen</SelectItem><SelectItem value="missing">Ohne Website</SelectItem></SelectContent></Select><Select value={leadSortMode} onValueChange={(value) => setLeadSortMode(value as LeadSortMode)}><SelectTrigger className="h-11 w-full border-white/10"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="updated">Zuletzt bearbeitet</SelectItem><SelectItem value="lead-score">Beste Leads zuerst</SelectItem><SelectItem value="website-opportunity">Schwächste Website zuerst</SelectItem></SelectContent></Select></div>
