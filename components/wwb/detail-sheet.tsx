...
import { WebsiteQualityScanner } from "./website-quality-scanner";
...
export function DetailSheet({
  ...
  onPatchLead: (id: string, patch: Partial<Pick<StoredLead, "status" | "priority" | "notes" | "nextAction" | "followUpAt" | "websiteStatus" | "audit" | "websiteReport" | "activities">>) => Promise<void>;
  ...
  const [websiteReport, setWebsiteReport] = useState<WebsiteQualityReport | null>(null);
  ...
  useEffect(() => {
    const resetTimer = window.setTimeout(() => {
      ...
      setWebsiteReport(lead?.websiteReport ?? business?.websiteReport ?? null);
      setImages([]);
    }, 0);
  ...
  const outreach = useMemo(() => (business ? outreachTemplatesFor({ ...business, websiteStatus, websiteReport: websiteReport ?? undefined }, prices, audit) : null), [audit, business, prices, websiteReport, websiteStatus]);
  ...
  async function saveChanges() {
    ...
    await onPatchLead(lead.id, {
      ...
      websiteReport: websiteReport ?? undefined,
    });
    setSaving(false);
  ...
  async function acceptWebsiteReport(nextReport: WebsiteQualityReport, nextAudit: WebsiteAudit) {
    setWebsiteReport(nextReport);
    setAudit(nextAudit);
    setWebsiteStatus("exists");
    if (lead && !claimedByOther) {
      await onPatchLead(lead.id, { websiteReport: nextReport, websiteStatus: "exists", audit: nextAudit });
    }
  }
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="wwb-detail-sheet w-full max-w-full overflow-x-hidden overflow-y-auto border-white/10 bg-[#0b0f14] p-0 sm:max-w-xl">
        ...
        <div className="space-y-6 p-5 sm:p-7">
          ...
          <BusinessResearchHub business={{ ...(lead ?? business), websiteStatus, audit, websiteReport: websiteReport ?? undefined }} />

          <WebsiteQualityScanner
            key={`website-scan-${business.id}`}
            business={{ ...(lead ?? business), websiteStatus, audit, websiteReport: websiteReport ?? undefined }}
            report={websiteReport}
            currentAudit={audit}
            savingBlocked={claimedByOther}
            onReport={acceptWebsiteReport}
          />
          ...
          {lead && (business.website || ["exists", "outdated", "unreachable"].includes(websiteStatus)) && (
            <section className="rounded-2xl border border-white/[.08] bg-white/[.02] p-4">
              <div className="flex items-start justify-between gap-4"><div><h3 className="flex items-center gap-2 text-base font-semibold"><CircleAlert className="size-4 text-[#d7b56d]" /> Manueller Audit-Feinschliff</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Automatische Ergebnisse kontrollieren oder jeden Punkt selbst setzen: offen → gut → Problem.</p></div><div className="rounded-xl bg-black/20 px-3 py-2 text-right"><span className="block text-[11px] text-muted-foreground">Audit-Score</span><strong className={auditScore === null ? "text-muted-foreground" : auditScore >= 70 ? "text-[#75e5b7]" : "text-[#ff9b78]"}>{auditScore === null ? "–" : `${auditScore}/100`}</strong></div></div>
          ...
          <section className="rounded-2xl border border-[#d7b56d]/15 bg-[#d7b56d]/[.035] p-4">
            <div><h3 className="flex items-center gap-2 text-base font-semibold"><Send className="size-4 text-[#d7b56d]" /> Kontakt-Studio</h3><p className="mt-1 text-xs text-muted-foreground">Firmenbezogene WWB-Vorlage wählen, persönlich prüfen und manuell verwenden.</p></div>
            {outreach && (
              <div className="mt-4 rounded-xl border border-white/[.08] bg-black/15 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-sm">Vertriebsfokus</strong><Badge variant="outline" className="border-[#52d6a0]/20 bg-[#52d6a0]/[.06] text-[#75e5b7]">Nutzen vor Angebot</Badge></div>
                <div className="mt-3 grid gap-2 sm:grid-cols-3">
                  <div className="rounded-lg bg-white/[.035] p-3"><span className="text-[11px] uppercase tracking-wider text-muted-foreground">Beobachtung</span><p className="mt-1 text-xs leading-5 text-foreground/80">{outreach.strategy.signal}</p></div>
                  <div className="rounded-lg bg-white/[.035] p-3"><span className="text-[11px] uppercase tracking-wider text-muted-foreground">Kundenvorteil</span><p className="mt-1 text-xs leading-5 text-foreground/80">{outreach.strategy.benefit}</p></div>
                  <div className="rounded-lg bg-white/[.035] p-3"><span className="text-[11px] uppercase tracking-wider text-muted-foreground">Gesprächsziel</span><p className="mt-1 text-xs leading-5 text-foreground/80">{outreach.strategy.nextStep}</p></div>
                </div>
                <p className="mt-2 text-[11px] leading-5 text-muted-foreground">Intern: {outreach.strategy.internalOffer}</p>
              </div>
            )}
            ...
            {outreach && <div role="note" className="mt-2 flex items-start gap-2 rounded-xl border border-[#ffbe96]/15 bg-[#ff9c67]/[.045] p-3 text-xs leading-5 text-muted-foreground"><CircleAlert className="mt-0.5 size-4 shrink-0 text-[#ffbe96]" /><p><strong className="text-foreground/80">Kontaktregel:</strong> {outreach.channelNotes[outreachMode]} Keine Rechtsberatung.</p></div>}
            <div className="mt-3 grid grid-cols-2 gap-2">
